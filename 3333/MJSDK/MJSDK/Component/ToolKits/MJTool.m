...
+ (NSString *)getIDFA:(NSString *)adSpaceID {
    LXIPManager *ipManager = [[LXIPManager alloc]init];
    if (ipManager.idfa.length <= 5) {
        NSLog(@"获取设备号失败，无法参与免费兑换道具，请开启权限后重试");
        MJExceptionReport *report = [MJExceptionReport reportWithADSpaceID:adSpaceID code:kMJSDKERRORGetIDFAFailure description:@"无法获取设备号"];
...
{
    if(!_keyWindow){
        _keyWindow = [[UIWindow alloc]initWithFrame:kMainScreen];
