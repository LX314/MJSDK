...
-(void)claimCouponsInfo {
    ...
    //领券
    [manager loadClaim:_mjShare.appsManager.adSpaceID claim:^(NSDictionary *params) {
        
        NSLog(@"领券返回params:%@",params);
        //无返回内容
        if (!params){
...
//            self.mjShare.mjGotoStepBlockBlock(KMJShareSixthStep);return;
            ...
            NSString *toastString = kMJNSLocalizedString(key, @"");
            NSBundle *bundle;[bundle localizedStringForKey:@"" value:@"" table:@""];
//            NSLocalizedStringFromTableInBundle(@"", @"Localizable", [[NSBundle mainBundle]pathForResource:@"MJSDK" ofType:@"bundle"], @"");
        ...
        [MJToast toast:@"发生了一个错误，领取红包失败" in:self];
        MJExceptionReport *report = [MJExceptionReport reportWithADSpaceID:_mjShare.appsManager.adSpaceID code:kMJSDKERRORCouponClaimFailure description:[NSString stringWithFormat:@"[优惠券][领取]失败:%@",error]];
...
//        [_imgView_content setContentMode:UIViewContentModeCenter];
    }
...
- (UIImageView *)imgView_logoLeft
{
    if(!_imgView_logoLeft){
        ...
        [_imgView_logoLeft setImage:[UIImage mj_imageNamed:@"券_01@2x.png"]];
    }
...
- (UIImageView *)imgView_logoRight
{
    if(!_imgView_logoRight){
        ...
        [_imgView_logoRight setImage:[UIImage mj_imageNamed:@"券_02@2x.png"]];
    }
