...
- (void)reset {
    ...
    [self.btn_modify setBackgroundImage:[UIImage mj_imageNamed:@"修改1@2x.png"] forState:UIControlStateNormal];
    [self.btn_modify setBackgroundImage:[UIImage mj_imageNamed:@"修改2@2x.png"] forState:UIControlStateHighlighted];
    [self.btn_modify setBackgroundImage:[UIImage mj_imageNamed:@"修改3@2x.png"] forState:UIControlStateDisabled];
...
- (void)modifyPhoneNumber {
    ...
    if ([MJTool judgePhoneNumber:changeNumber] && ![[MJTool getMJShareNewTel] isEqualToString:changeNumber]) {
    ...
    } else if ([[MJTool getMJShareNewTel] isEqualToString:changeNumber]){
        
        [MJToast toast:@"您输入的手机号重复" in:self];
