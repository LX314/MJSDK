...
//    [self.lab_content setText:@"梦幻般的真实\nRPG大作"];
    ...
    [self.imageView_show mj_setImageWithURLString:image placeholderImage:[UIImage imageNamed:kMJSDKAPPREShowIMGPlaceholderImageName] impAds:impAds success:nil failure:nil];
//    [self.imageView_show setImageWithURL:[NSURL URLWithString:image] placeholderImage:[UIImage imageNamed:kMJSDKAPPREShowIMGPlaceholderImageName]];
    [self.imageView_logo mj_setImageWithURLString:logUrl placeholderImage:[UIImage imageNamed:kMJSDKAPPRELogoIMGPlaceholderImageName] impAds:impAds success:nil failure:nil];
//    [self.imageView_logo setImageWithURL:[NSURL URLWithString:logUrl] placeholderImage:[UIImage imageNamed:kMJSDKAPPRELogoIMGPlaceholderImageName]];
   ...
   __block MJImpAds *impAds = _mjResponse.impAds[self.selfIndexPath.row];
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [UIImage mj_setImageWithURLString:share_image_url placeholderImage:nil success:^(NSURLRequest * _Nullable request, NSHTTPURLResponse * _Nullable response, UIImage * _Nullable image) {
            ...
            } title:share_title description:share_subtitle imagesArray:@[share_image] url:landingPageUrl viewController:self.appsManager];
...
- (void)masonry {
    ...
    [self.imageView_logo mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        //        make.height.equalTo(self.imageView_logo.mas_width).multipliedBy(45.f/150.f);
    }];
...
-(UIImageView *)imageView_share {
    ...
    if (!_imageView_share) {
        ...
        [_imageView_share setImage:[UIImage imageNamed:@"已分享"]];
