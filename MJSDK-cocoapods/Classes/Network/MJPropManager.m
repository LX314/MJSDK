...
//    NSString *json = [MJTool toJsonString:params];
    [netWorking POST:@"http://v.mjmobi.com/pq" params:params success:^(NSURLSessionDataTask * _Nonnull dataTask, id  _Nullable responseObject) {
        /**status: 状态
         *  1: 未领取
        ...
        MJExceptionReport *report = [MJExceptionReport reportWithADSpaceID:@"" code:kMJSDKERRORPropQueryFailure description:[NSString stringWithFormat:@"[道具][查询]失败:%@", error]];
...
+ (void)claimPropID:(NSString *)propID price:(NSInteger)price propBlock:(kMJPropManagerBlock)propBlock {
    ...
    [netWorking POST:kMJSDKPropClaimAPI params:params success:^(NSURLSessionDataTask * _Nonnull dataTask, id  _Nullable responseObject) {
        ...
        MJExceptionReport *report = [MJExceptionReport reportWithADSpaceID:@"" code:kMJSDKERRORPropClaimFailure description:[NSString stringWithFormat:@"[道具][领取]失败:%@", error]];
        [MJExceptionReportManager uploadOnlineExceptionReport:@[report]];
    }];
}
/**
