 ...
 */
- (void)loadQuery:(NSString *)adSpaceID query:(kMJShareBlock)queryBlock {
    ...
    NSLog(@"%@",queryParams);
    [networking POST:urlString params:queryParams success:^(NSURLSessionDataTask * _Nonnull dataTask, id  _Nullable responseObject) {
        
        NSString * coupons_code = responseObject[@"coupons_info"][@"coupons_code"];
        if (coupons_code.length <= 0) {
            MJExceptionReport *report = [MJExceptionReport reportWithADSpaceID:adSpaceID code:kMJSDKERRORGetCouponsCodeFailure description:[NSString stringWithFormat:@"[优惠券][券号]获取失败"]];
        ...
        if ([MJTool judgeURLString:image_url] == NO) {
            MJExceptionReport *report = [MJExceptionReport reportWithADSpaceID:adSpaceID code:kMJSDKERRORGetCouponImageUrlFailure description:[NSString stringWithFormat:@"[优惠券][券图片]URL异常"]];
            [MJExceptionReportManager uploadOnlineExceptionReport:@[report]];
            return;
        }
        
        if ([MJTool judgeURLString:logo_url] == NO) {
            MJExceptionReport *report = [MJExceptionReport reportWithADSpaceID:adSpaceID code:kMJSDKERRORGetCouponLogoUrlFailure description:[NSString stringWithFormat:@"[优惠券][logo]URL异常"]];
            [MJExceptionReportManager uploadOnlineExceptionReport:@[report]];
            return;
        }
        if (queryBlock) {
        ...
        ////
        MJExceptionReport *report = [MJExceptionReport reportWithADSpaceID:adSpaceID code:kMJSDKERRORCouponQueryFailure description:[NSString stringWithFormat:@"[优惠券][查询]失败:%@",error]];
        [MJExceptionReportManager uploadOnlineExceptionReport:@[report]];
    }];
...
/**
  ...
  */
- (void)loadClaim:(NSString * _Nonnull)adSpaceID claim:(kMJShareBlock)claimBlock failure:(kMJClaimFailedBlock)failureBlock couponCode:(NSString *)code telNumber:(NSString *)tel {
    LXNetWorking *networking = [LXNetWorking manager];
    ...
    [networking POST:urlString params:params success:^(NSURLSessionDataTask * _Nonnull dataTask, id  _Nullable responseObject) {
        //
        if (!params) {
            MJExceptionReport *report = [MJExceptionReport reportWithADSpaceID:adSpaceID code:kMJSDKERRORADNOResponseData description:[NSString stringWithFormat:@"领取优惠券返回数据为空"]];
            [MJExceptionReportManager uploadOnlineExceptionReport:@[report]];
        }
        if (claimBlock) {
 ...
 */
- (void)loadUpdate:(NSString * _Nonnull)adSpaceID update:(kMJShareBlock _Nonnull)updateBlock updatephoneNumber:(NSString *_Nonnull)phoneNumber{
    ...
    [networking POST:urlString params:params success:^(NSURLSessionDataTask * _Nonnull dataTask, id  _Nullable responseObject) {
        ...
        MJExceptionReport *report = [MJExceptionReport reportWithADSpaceID:adSpaceID code:kMJSDKERRORCouponUpdateFailure description:[NSString stringWithFormat:@"[优惠券][手机号修改]失败:%@",error]];
        [MJExceptionReportManager uploadOnlineExceptionReport:@[report]];
    }];
}
#pragma mark - params
