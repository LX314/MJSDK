...
- (void)loadData:(NSDictionary *)params adType:(KMJADType)adType successBlock:(kRequestSuccessBlock)successBlock errorBlock:(kRequestErrorBlock)errorBlock {
    ...
    NSString *adSpaceID = params[@"impression"][@"adloc_code"];
    //封装 request
    ...
    NSURLSessionDataTask *dataTask = [manager dataTaskWithRequest:murequest completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        kMJSDKADSpaceID = adSpaceID;
        if (error) {
            MJExceptionReport *report = [MJExceptionReport reportWithADSpaceID:adSpaceID code:kMJSDKERRORPOSTRequestFailure description:[NSString stringWithFormat:@"请求 %@ 类型数据时发生错误\n\nerror.localizedDescription:%@", KNSStringFromMJADType(adType), error.localizedDescription]];
...
//        }
        if (mjResponse.code != 100)
        {
            ...
            MJExceptionReport *report = [MJExceptionReport reportWithADSpaceID:adSpaceID code:kMJSDKERRORResponseSuccessButNotFitCode description:[NSString stringWithFormat:@"请求成功,但 code 显示为异常,old_response:%@", [responseObject description]]];
        ...
        if (mjResponse.impAds.count <= 0) {
            ...
            MJExceptionReport *report = [MJExceptionReport reportWithADSpaceID:adSpaceID code:error_t.code description:[NSString stringWithFormat:@"%@", error]];
            [MJExceptionReportManager uploadOnlineExceptionReport:@[report]];
            MJNSLog(@"errcode:%ld", report.code);
            return;
        }
