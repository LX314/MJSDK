...
@interface MJExceptionReportManager()
{
...
+ (void)uploadOnlineExceptionReport:(NSArray<MJExceptionReport *> *)exceptionReports {
    if (exceptionReports.count == 1) {
        MJExceptionReport *report = [exceptionReports firstObject];
        NSLog(@"--->>exceptionReports:\n\
              code:%ld\n\
              description:%@\n\
              msg:%@", report.code, report.desc, report.msg);
    } else {
        NSLog(@"--->>exceptionReports:%@", exceptionReports);
    }
    NSString *urlString = kMJSDKErrorReportAPI;
    NSDictionary *params;
    ...
    [[LXNetWorking manager] basePOST:urlString params:params exceptionReport:YES success:^(NSURLSessionDataTask * _Nonnull dataTask, id  _Nullable responseObject) {
        ...
        MJExceptionReport *report_t = [exceptionReports firstObject];
        MJExceptionReport *report = [MJExceptionReport reportWithADSpaceID:report_t.msg.adspaceId code:kMJSDKERROROnlineExceptionDataUploadFailure description:[NSString stringWithFormat:@"异常上报[在线](POST)-failure:%@",error]];
 ...
 *  @param impAds impAds
 */
+ (void)insertOffLineExposureReport:(MJImpAds *)impAds adSpaceID:(NSString *)adSpaceID {
    //
    NSArray *array = [impAds.showUrl componentsSeparatedByString:@"?"];
    if (array.count != 2) {
        MJExceptionReport *report = [MJExceptionReport reportWithADSpaceID:adSpaceID code:kMJSDKERRORURLFailure description:@"曝光 URL 异常,获取 ? 时失败"];
 ...
 */
+ (void)uploadOnlineExposureReport:(MJImpAds *)impAds adSpaceID:(NSString *)adSpaceID success:(kMJExposureSuccessBlock)successBlock failure:(kMJExposureFailureBlock)failureBlock {
    ...
    [[LXNetWorking manager]GET:exposureURL params:nil success:^(NSURLSessionDataTask * _Nonnull dataTask, id  _Nullable responseObject) {
        ...
        //1.离线曝光
        [self insertOffLineExposureReport:impAds adSpaceID:adSpaceID];
        //2.异常上报
        NSLog(@"曝光[在线](GET)-失败,开始异常上报:%@\n\nimpression_seq_no:%ld",error, impression_seq_no);
        MJExceptionReport *report = [MJExceptionReport reportWithADSpaceID:adSpaceID code:kMJSDKERROROnlineExposureFailure description:[NSString stringWithFormat:@"曝光(GET)-failure:%@\n\nimpression_seq_no:%ld\n\nexposureURL:%@",error, impression_seq_no, exposureURL]];
        [MJExceptionReportManager uploadOnlineExceptionReport:@[report]];
        if (failureBlock) {
...
/**
 外链跳转[landing-page-url]

 @param landingPageUrl landingPageUrl
 @param completion     completion
 */
+ (void)mjGotoLandingPageUrl:(NSString *)landingPageUrl before:(void(^)(void))before completion:(void(^)(void))completion {
    NSLog(@"goto landingPageURL:%@", landingPageUrl);
    if ([MJExceptionReportManager validateAndExceptionReport:landingPageUrl]) {
        if (before) {
            before();
        }
//#warning TODO suspend|resumed
        MJWebView *mjWebview = [[MJWebView alloc]init];;
        mjWebview.parentVC = [MJTool getTopVC];
        mjWebview.completion = completion;
        [mjWebview requestUrl:landingPageUrl];
        [mjWebview presentWebView];
    }
}
 ...
 *  @param failed    failed
 */
+ (void)goodInfoReport:(NSString *)reportURL adSpaceID:(NSString *)adSpaceID success:(SuccessBlockType)success failed:(FailedBlockType)failed {
    ...
    [[LXNetWorking manager]GET:reportURL params:nil success:^(NSURLSessionDataTask * _Nonnull dataTask, id  _Nullable responseObject) {
        ...
        MJExceptionReport *report = [MJExceptionReport reportWithADSpaceID:adSpaceID code:kMJSDKERRORGoodInfoReportFailure description:[NSString stringWithFormat:@"好货推荐上报失败:%@", error]];
        [MJExceptionReportManager uploadOnlineExceptionReport:@[report]];
        if (failed) {
            failed(dataTask,error);
        }
